...
describe('карточки заданий', () => {
  ...
  it('показывает план ответа только когда он запрошен', () => { const question = { ...open('o'), answerPlan: ['Дать определение', 'Привести пример'] }; render(<OpenCard question={question} draft="" showAnswerPlan onDraft={vi.fn()} onAnswer={vi.fn()} onRate={vi.fn()}/>); expect(screen.getByRole('heading', { name: 'План ответа' })).toBeInTheDocument(); expect(screen.getByText('Привести пример')).toBeInTheDocument(); });
  it('рендерит LaTeX-формулу в читаемом виде', () => { const question = { ...mcq('math'), question: 'Вычислите $\\frac{1}{2}$' }; const { container } = render(<McqCard question={question} onAnswer={vi.fn()}/>); expect(container.querySelector('.katex')).toBeInTheDocument(); expect(container.querySelector('.katex-html')?.textContent).toContain('1'); });
